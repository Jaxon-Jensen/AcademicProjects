...
int main(int argc, char *argv[])
{
    ...
    /**/
    port_str = argv[1]; /* This could be in 'argv[1]', for port. */

    /* Convert the numeric string into an unsigned short int */
    port = (unsigned short int)atoi(port_str);

    /* Next, prepare the Internet address, and set it to 127.0.0.1 in Network-byte order.
    Note we chose to set the member 's_addr' directly. */
